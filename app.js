...
// =======================
// LIVE REVEAL — speaker-controlled answer reveal
// =======================
//
// When liveMode is ON, participants see a "Waiting for reveal" screen after
// submitting an answer. They poll the worker every few seconds, and once the
// admin marks their current question key as revealed, the feedback screen
// (which was already populated) is shown.
//
// State shape (in KV via worker):
//   { liveMode: bool, revealedKeys: [string], updatedAt: number }
// =======================
...
async function fetchRevealState() {
  try {
    ...
    console.warn("fetchRevealState failed:", e.message);
    return null;
  }
}
...
function startLivePoll() {
  ...
  fetchRevealState().then(applyRevealStateChange);
...
// Called whenever poll returns. If we're on the waiting screen and our
// current question is now revealed (or live mode was turned off), flip to
// the feedback screen.
function applyRevealStateChange() {
...
// Replaces the old `showScreen("feedback")` in the submit functions.
// If live mode is on AND this question isn't already revealed, show the
// waiting screen and keep polling. Otherwise show feedback immediately.
function presentFeedback() {
  // Save progress regardless
  try { saveProgress(); } catch(e) {}

  const key = answerKey();

  // If we already know it's revealed (or live mode is off), straight to feedback
  if (!liveMode || revealedKeys.has(key)) {
    showScreen("feedback");
    return;
  }

  // Otherwise: show waiting screen and ensure poll is running
  const waitingAnswerBox  = document.getElementById("waitingYourAnswer");
  const waitingAnswerText = document.getElementById("waitingYourAnswerText");
  const stored = userAnswers[key];
  if (waitingAnswerBox && waitingAnswerText && stored && stored.userAnswers && stored.userAnswers.length) {
    const q = getCurrentQuestion();
    let display = stored.userAnswers.join(", ");
    // Translate codes (A, B, C…) to labels if available
    ...
    waitingAnswerText.innerText = display;
    waitingAnswerBox.style.display = "block";
  } else if (waitingAnswerBox) {
    waitingAnswerBox.style.display = "none";
  }
...
// =======================
// ADMIN — Live Reveal Controls
// =======================

let adminRevealCache = { liveMode: false, revealedKeys: [] };

async function adminLoadRevealState() {
  const data = await fetchRevealState();
  if (data) {
    adminRevealCache = { liveMode: !!data.liveMode, revealedKeys: data.revealedKeys || [] };
  }
  const toggle = document.getElementById("adminLiveMode");
  if (toggle) toggle.checked = adminRevealCache.liveMode;
  adminRenderRevealList();
}

async function adminSaveRevealState(opts) {
  const code = window.adminCode;
  if (!code) { adminRevealStatus("Not authenticated — please log in to admin again.", true); return; }
  const payload = {
    action: "setRevealState",
    adminCode: code,
    liveMode: opts.liveMode !== undefined ? opts.liveMode : adminRevealCache.liveMode,
    revealedKeys: opts.revealedKeys !== undefined ? opts.revealedKeys : adminRevealCache.revealedKeys
  };
  try {
    const res = await fetch(PROXY_URL, {
      method: "POST",
    ...
    adminRevealCache = { liveMode: !!data.state.liveMode, revealedKeys: data.state.revealedKeys || [] };
    liveMode = adminRevealCache.liveMode;
    revealedKeys = new Set(adminRevealCache.revealedKeys);
    adminRevealStatus("✓ Updated · " + new Date().toLocaleTimeString());
    adminRenderRevealList();
  } catch (e) {
...
function adminToggleLiveMode() {
  const checked = document.getElementById("adminLiveMode").checked;
  adminSaveRevealState({ liveMode: checked });
}

function adminToggleReveal(key) {
  const set = new Set(adminRevealCache.revealedKeys);
  if (set.has(key)) set.delete(key); else set.add(key);
  adminSaveRevealState({ revealedKeys: Array.from(set) });
}

function adminRevealAll() {
  if (!confirm("Reveal ALL answers to all participants?")) return;
  const allKeys = adminCollectAllKeys();
  adminSaveRevealState({ revealedKeys: allKeys });
}

function adminClearReveals() {
  if (!confirm("Hide all answers again? Participants currently viewing feedback will stay where they are.")) return;
  adminSaveRevealState({ revealedKeys: [] });
}

// Walk all cases + nested questions and return their answer-keys
function adminCollectAllKeys() {
  const keys = [];
...
function adminRenderRevealList() {
  ...
  const revealedSet = new Set(adminRevealCache.revealedKeys);
  let html = "";

  cases.forEach((c, cIdx) => {
    html += `<div style="padding:10px 14px;background:var(--surface2);border-bottom:1px solid var(--border);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:var(--muted);">Case ${cIdx + 1} · ${escapeHtml((c.intro || "Untitled").slice(0, 70))}</div>`;
    function renderRow(q, path, depth) {
      ...
      html += `
        <div style="display:flex;align-items:center;gap:10px;padding:9px 14px 9px ${14 + indent}px;border-bottom:1px solid var(--border);background:${isRevealed ? "rgba(92,45,126,0.06)" : "var(--surface)"};">
...
// Hook into switchAdminTab so the live tab refreshes when opened
(function hookLiveAdminTab() {
  if (typeof switchAdminTab !== "function") return;
  const orig = switchAdminTab;
  switchAdminTab = function(tab) {
    orig(tab);
...
// Start poll whenever we enter course screens, stop on exit
(function hookShowScreenForPoll() {
  if (typeof showScreen !== "function") return;
  const orig = showScreen;
  const courseScreens = new Set(["caseIntro","question","feedback","waiting","results","review"]);
  showScreen = function(id) {
    orig(id);
    if (courseScreens.has(id)) startLivePoll(); else stopLivePoll();
  };
})();
