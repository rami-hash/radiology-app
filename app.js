      ...
      .map(c => ({
        ...
        questions: (c.questions || [])
          ...
          .map(q => ({
            ...
            allowMultiSelect: q.allowMultiSelect || false,
...
let ftSuggestions    = [];
let ftNumRequired    = 1;
let ftAllowMulti     = false; // set per question from allowMultiSelect field

function buildFreetextFields(numFields, suggestions) {
  ftSelectedTags  = [];
  ftSuggestions   = suggestions.slice();
  ftNumRequired   = numFields; // kept for scoring, NOT shown to candidate
  ...
  boxLabel.innerText = "Select your answer(s) from the list below:";
  ...
  // ── Counter — shows selected count only, no target ────
  ...
  suggestions.forEach(s => {
    ...
    li.addEventListener("click", () => {
      ftAddTag(s);
      // Auto-submit in single mode after selection
      if (!ftAllowMulti) {
        setTimeout(() => submitFreetext(), 300);
      }
    });
    list.appendChild(li);
  });
  ...
  // Show/hide submit button based on mode
  const submitBtn = document.getElementById("freetextSubmit");
  if (submitBtn) submitBtn.style.display = ftAllowMulti ? "block" : "none";

  // Update label
  boxLabel.innerText = ftAllowMulti
    ? "Select your answer(s) from the list below:"
    ...
    <div id="cmMultiSelectSection" style="display:${qtype==="freetext"?"none":"block"}">
      <div class="cm-field">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          <label style="display:flex;align-items:center;gap:8px;padding:10px 16px;border-radius:10px;border:2px solid ${!q.allowMultiSelect?"var(--accent)":"var(--border)"};background:${!q.allowMultiSelect?"rgba(92,45,126,0.08)":"var(--surface2)"};cursor:pointer;font-size:14px;font-family:'DM Sans',sans-serif;transition:all 0.15s;">
            <input type="radio" name="cmF_multiSelect" id="cmF_single" value="single" ${!q.allowMultiSelect?"checked":""} onchange="cmUpdateSelectionMode()" style="accent-color:var(--accent);">
            <div>
          ...
          <label style="display:flex;align-items:center;gap:8px;padding:10px 16px;border-radius:10px;border:2px solid ${q.allowMultiSelect?"var(--accent)":"var(--border)"};background:${q.allowMultiSelect?"rgba(92,45,126,0.08)":"var(--surface2)"};cursor:pointer;font-size:14px;font-family:'DM Sans',sans-serif;transition:all 0.15s;">
            <input type="radio" name="cmF_multiSelect" id="cmF_multi" value="multi" ${q.allowMultiSelect?"checked":""} onchange="cmUpdateSelectionMode()" style="accent-color:var(--accent);">
            <div>
      ...
      <div class="cm-field">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          <label style="display:flex;align-items:center;gap:8px;padding:10px 16px;border-radius:10px;border:2px solid ${!q.allowMultiSelect?"var(--accent)":"var(--border)"};background:${!q.allowMultiSelect?"rgba(92,45,126,0.08)":"var(--surface2)"};cursor:pointer;font-size:14px;font-family:'DM Sans',sans-serif;transition:all 0.15s;">
            ...
            <div>
              <div style="font-weight:600;">Single answer</div>
              <div style="font-size:11px;color:var(--muted);">One selection only</div>
            </div>
          </label>
          <label style="display:flex;align-items:center;gap:8px;padding:10px 16px;border-radius:10px;border:2px solid ${q.allowMultiSelect?"var(--accent)":"var(--border)"};background:${q.allowMultiSelect?"rgba(92,45,126,0.08)":"var(--surface2)"};cursor:pointer;font-size:14px;font-family:'DM Sans',sans-serif;transition:all 0.15s;">
            <input type="radio" name="cmF_multiSelect" id="cmF_multi" value="multi" ${q.allowMultiSelect?"checked":""} onchange="cmUpdateSelectionMode()" style="accent-color:var(--accent);">
            <div>
              <div style="font-weight:600;">Multiple answers</div>
              <div style="font-size:11px;color:var(--muted);">Candidate selects one or more</div>
            </div>
          </label>
        </div>
      </div>
...
function cmUpdateSelectionMode() {
  const isSingle = document.getElementById("cmF_single")?.checked;
  const labels   = document.querySelectorAll("label[for='cmF_single'], label[for='cmF_multi']");
  document.querySelectorAll("input[name='cmF_multiSelect']").forEach(radio => {
    ...
    lbl.style.background  = active ? "rgba(92,45,126,0.08)"   : "var(--surface2)";
  });
}
...
function cmSaveQuestion(isEdit) {
  ...
  const urlField       = document.getElementById("cmF_imageUrl") ? document.getElementById("cmF_imageUrl").value.trim() : "";
  const b64Field       = document.getElementById("cmF_imageBase64") ? document.getElementById("cmF_imageBase64").value : "";
  const allowMultiSel  = document.getElementById("cmF_multi") ? document.getElementById("cmF_multi").checked : false;
...
function cmSaveSubQuestion() {
  ...
  const allowMultiSel  = document.getElementById("cmSQ_multi") ? document.getElementById("cmSQ_multi").checked : false;
