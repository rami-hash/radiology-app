...
(function() {
  ...
  s.textContent = `
    .chip-option-list { display:flex !important; flex-direction:column !important; gap:8px !important; max-height:340px !important; overflow-y:auto !important; padding:2px !important; margin-top:6px !important; background:none !important; border:none !important; border-radius:0 !important; box-shadow:none !important; }
    .chip-option-item { display:flex !important; align-items:center !important; padding:14px 18px !important; font-size:14px !important; color:#1e0a2e !important; cursor:pointer !important; background:#ffffff !important; border:1.5px solid #b8a8d4 !important; border-radius:10px !important; transition:all 0.18s !important; }
    .chip-option-item:hover { border-color:#5c2d7e !important; color:#5c2d7e !important; background:rgba(92,45,126,0.06) !important; }
    .chip-option-item.chip-option-selected { background:rgba(92,45,126,0.12) !important; border-color:#5c2d7e !important; border-width:2px !important; font-weight:600 !important; color:#5c2d7e !important; box-shadow:0 0 0 3px rgba(92,45,126,0.15) !important; }
    #ftSuggestionList { display:flex !important; flex-direction:column !important; gap:8px !important; max-height:220px !important; overflow-y:auto !important; padding:2px !important; margin-top:6px !important; background:none !important; border:none !important; border-radius:0 !important; list-style:none !important; }
    .freetext-list-item, .ft-suggestion-item { padding:14px 18px !important; font-size:14px !important; color:#1e0a2e !important; cursor:pointer !important; background:#ffffff !important; border:1.5px solid #b8a8d4 !important; border-radius:10px !important; transition:all 0.18s !important; list-style:none !important; }
    .freetext-list-item:hover, .ft-suggestion-item:hover { border-color:#5c2d7e !important; color:#5c2d7e !important; background:rgba(92,45,126,0.06) !important; }

    /* ── Review screen ── */
    .review-summary { display:flex; align-items:center; justify-content:center; gap:0; background:linear-gradient(135deg,#2a0a4a,#5c2d7e); border-radius:16px; padding:28px 32px; margin-bottom:32px; flex-wrap:wrap; }
    .review-summary-stat { text-align:center; padding:0 28px; }
    .review-summary-num { font-family:'JetBrains Mono',monospace; font-size:32px; font-weight:700; color:#fff; line-height:1; margin-bottom:4px; }
    .review-summary-lbl { font-size:11px; font-weight:600; text-transform:uppercase; letter-spacing:2px; color:rgba(255,255,255,0.55); }
    .review-summary-divider { width:1px; height:52px; background:rgba(255,255,255,0.15); flex-shrink:0; }
    .review-case { margin-bottom:28px; }
    .review-case-header { font-family:'DM Serif Display',serif; font-size:18px; color:var(--text); padding:14px 20px; background:var(--surface); border:1px solid var(--border); border-radius:12px 12px 0 0; border-bottom:none; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; }
    .review-case-stats { display:flex; align-items:center; gap:16px; padding:12px 20px; background:var(--surface2); border:1px solid var(--border); border-radius:0 0 12px 12px; margin-bottom:14px; }
    .stats-bar-wrapper { flex:1; display:flex; align-items:center; gap:12px; }
    .stats-label { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:var(--muted); white-space:nowrap; }
    .stats-bar-track { flex:1; height:8px; background:var(--border); border-radius:4px; overflow:hidden; }
    .stats-bar-fill { height:100%; background:linear-gradient(90deg,var(--accent),var(--accent2)); border-radius:4px; transition:width 0.6s ease; }
    .stats-score { font-family:'JetBrains Mono',monospace; font-size:13px; font-weight:700; color:var(--accent); white-space:nowrap; }
    .review-question { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:18px 20px; margin-bottom:10px; }
    .review-q-text { font-size:14px; font-weight:600; color:var(--text); margin-bottom:12px; line-height:1.5; }
    .review-answer-row { display:flex; align-items:baseline; gap:10px; margin-bottom:8px; flex-wrap:wrap; font-size:14px; color:var(--text); }
    .review-tag { display:inline-block; font-size:11px; font-weight:700; padding:3px 10px; border-radius:20px; white-space:nowrap; flex-shrink:0; }
    .review-tag.user-ok { background:rgba(46,125,50,0.1); color:#2e7d32; border:1px solid #2e7d32; }
    .review-tag.user-bad { background:rgba(192,57,43,0.08); color:#c0392b; border:1px solid #c0392b; }
    .review-tag.correct { background:rgba(92,45,126,0.1); color:#5c2d7e; border:1px solid #5c2d7e; }
    .review-points { font-family:'JetBrains Mono',monospace; font-size:13px; font-weight:700; margin-top:10px; }
    .review-explanation { font-size:13px; color:var(--muted); margin-top:10px; padding-top:10px; border-top:1px solid var(--border); line-height:1.65; font-style:italic; }
  `;
...
// ── Point this to your server URL ──────────────────────────────────────────
// Local dev:   "http://localhost:3000"
// Production:  "https://yourserver.com/api"  (if nginx proxies /api → port 3000)
const PROXY_URL = "http://187.33.153.170:8080/api";
...
function loadCases() {
  ...
  .then(data => {
    ...
    if (cases.length === 0) {
      ...
      if (errEl) { errEl.textContent = "No cases found. Please add cases via the Admin → Content panel."; errEl.style.display = "block"; }
      return;
    }
    ...
    // Restore session based on what screen the user was on before refresh
    const lastScreen = sessionStorage.getItem("currentScreen") || "welcome";
    const savedAdmin = sessionStorage.getItem("adminCode");
    const savedRole  = sessionStorage.getItem("adminRole");
    const savedName  = sessionStorage.getItem("playerName");
    const savedEmail = sessionStorage.getItem("playerEmail");

    // Restore admin panel
    if (savedAdmin && lastScreen === "admin") {
      if (loading) loading.classList.remove("active");
      window.adminCode = savedAdmin;
      _adminCode = savedAdmin;
      showScreen("admin");
      loadAdminData();
      return;
    }

    // Restore speaker panel
    if (savedAdmin && lastScreen === "speaker") {
      if (loading) loading.classList.remove("active");
      window.adminCode = savedAdmin;
      _adminCode = savedAdmin;
      _speakerCode = savedAdmin;
      showScreen("speaker");
      speakerRefresh();
      return;
    }

    // Restore candidate course
    if (savedName && savedEmail) {
      playerName  = savedName;
      playerEmail = savedEmail;
      loadProgress(savedEmail).then(async saved => {
        if (loading) loading.classList.remove("active");
        await fetchRevealState();
        if (liveMode && !liveRevealState.courseStarted) {
          startCourseTimer(); updateProgress();
...
// TIME TRACKING
// =======================
function startQuestionTimer() { } // timing removed
// Fire-and-forget: save answer choice to DB for analytics
function saveAnswerToServer(questionKey, q, c, selectedAnswer, isCorrect) {
  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...
      questionText:   q ? (q.text || "").slice(0, 120) : "",
      caseTitle:      c ? (c.intro || "").slice(0, 80) : "",
...
let _analyticsData = []; // cached for export

function exportAnswerAnalyticsCSV() {
  if (!_analyticsData.length) { alert("No data to export. Click Refresh first."); return; }
  function fmtKey(key) {
    const parts = key.split("_");
    return "C" + (parseInt(parts[0])+1) + "-Q" + parts.slice(1).map(n => parseInt(n)+1).join(".");
  }
  let csv = "Question,Case,Answer,Correct,Count,Percentage\n";
  _analyticsData.forEach(r => {
    csv += `"${fmtKey(r.question_key)}","${(r.case_title||"").replace(/"/g,'""')}","${String(r.selected_answer).replace(/"/g,'""')}",${r.is_correct?"Yes":"No"},${r.count},${r.percentage}%\n`;
  });
  const blob = new Blob([csv], { type: "text/csv" });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement("a");
  a.href = url; a.download = "answer_analytics_" + new Date().toISOString().slice(0,10) + ".csv";
  document.body.appendChild(a); a.click();
  document.body.removeChild(a); URL.revokeObjectURL(url);
}

function exportAnswerAnalyticsXLSX() {
  if (!_analyticsData.length) { alert("No data to export. Click Refresh first."); return; }
  function fmtKey(key) {
    const parts = key.split("_");
    return "C" + (parseInt(parts[0])+1) + "-Q" + parts.slice(1).map(n => parseInt(n)+1).join(".");
  }
  const wsData = [["Question","Question Text","Case","Answer","Correct","Count","Percentage"]];
  _analyticsData.forEach(r => {
    wsData.push([fmtKey(r.question_key), r.question_text||"", r.case_title||"", String(r.selected_answer), r.is_correct?"Yes":"No", r.count, r.percentage + "%"]);
  });
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  ws["!cols"] = [{wch:10},{wch:50},{wch:30},{wch:30},{wch:8},{wch:8},{wch:10}];
  XLSX.utils.book_append_sheet(wb, ws, "Answer Analytics");
  XLSX.writeFile(wb, "answer_analytics_" + new Date().toISOString().slice(0,10) + ".xlsx");
}

function loadAnswerAnalytics() {
  const body = document.getElementById("answerAnalyticsBody");
  if (!body) return;
  body.innerHTML = "<p style='color:var(--muted);font-size:13px;text-align:center;padding:24px;'>Loading…</p>";

  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "getAnswerAnalytics", adminCode: window.adminCode || "" })
  })
  .then(r => r.json())
  .then(rows => {
    _analyticsData = Array.isArray(rows) ? rows : [];
    if (!_analyticsData.length) {
      body.innerHTML = "<p style='color:var(--muted);font-size:13px;text-align:center;padding:24px;'>No answer data yet — data appears as candidates answer questions.</p>";
      return;
    }
    const rows2 = _analyticsData;

    // Group rows by question_key
    const questions = {};
    rows2.forEach(r => {
      if (!questions[r.question_key]) {
        questions[r.question_key] = { key: r.question_key, text: r.question_text, caseTitle: r.case_title, answers: [] };
      }
      questions[r.question_key].answers.push(r);
    });

    function fmtKey(key) {
      const parts = key.split("_");
      const c = parseInt(parts[0]) + 1;
      const q = parts.slice(1).map(n => parseInt(n) + 1).join(".");
      return "C" + c + " · Q" + q;
    }

    body.innerHTML = Object.values(questions).map(q => {
      const total = q.answers.reduce((s, a) => s + Number(a.count), 0);
      const mostCommon = q.answers[0]; // already sorted by count DESC

      const bars = q.answers.map(a => {
        const pct     = Number(a.percentage);
        const correct = a.is_correct;
        const isMost  = a === mostCommon;
        const color   = correct ? "var(--accent)" : "var(--danger)";
        const icon    = correct ? "✅" : "❌";
        return `
          <div style="margin-bottom:10px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
              <span style="font-size:12px;font-weight:${isMost?"700":"400"};color:var(--text);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(String(a.selected_answer))}</span>
              <span style="font-size:11px;color:${color};font-weight:700;">${icon}</span>
              <span style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--muted);min-width:40px;text-align:right;">${pct}% (${a.count})</span>
            </div>
            <div style="height:8px;background:var(--surface2);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${pct}%;background:${color};border-radius:4px;opacity:${isMost?"1":"0.5"};transition:width 0.5s;"></div>
            </div>
          </div>`;
      }).join("");

      const mostIcon  = mostCommon.is_correct ? "✅" : "❌";
      const mostColor = mostCommon.is_correct ? "rgba(46,125,50,0.1)" : "rgba(192,57,43,0.07)";
      const mostBorder= mostCommon.is_correct ? "#2e7d32" : "var(--danger)";
      ...
      return `
        <div style="margin-bottom:20px;border:1px solid var(--border);border-radius:12px;overflow:hidden;">
          <div style="padding:12px 16px;background:var(--surface2);border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <span style="font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:700;color:var(--accent);background:rgba(92,45,126,0.1);border:1px solid rgba(92,45,126,0.2);padding:2px 8px;border-radius:20px;">${fmtKey(q.key)}</span>
            <span style="font-size:13px;font-weight:600;color:var(--text);flex:1;">${escapeHtml((q.text||"").slice(0,80))}</span>
            <span style="font-size:11px;color:var(--muted);">${total} response${total!==1?"s":""}</span>
          </div>
          <div style="padding:14px 16px;">
            ${bars}
            <div style="margin-top:12px;padding:10px 14px;border-radius:8px;background:${mostColor};border:1px solid ${mostBorder};font-size:12px;color:var(--text);">
              <strong>Most answered:</strong> "${escapeHtml(String(mostCommon.selected_answer))}" — ${mostIcon} ${mostCommon.is_correct ? "Correct" : "Incorrect"} · chosen by ${mostCommon.percentage}% of candidates
            </div>
          </div>
        </div>`;
    }).join("");
  ...
  .catch(() => {
    body.innerHTML = "<p style='color:var(--danger);font-size:13px;text-align:center;padding:24px;'>Failed to load data.</p>";
  });
}

function recordQuestionTime() { } // timing removed

document.addEventListener("DOMContentLoaded", () => {
  loadCases();
  loadCourseSettings().then(s => {
    const mins = (s && s.courseDuration) ? Number(s.courseDuration) : 45;
    const display = document.getElementById("courseTimeDisplay");
    if (display) display.innerText = formatTime(mins * 60);
    // Update welcome screen stats
    const durEl = document.getElementById("wlDurNum");
    if (durEl) durEl.textContent = mins;
  });
});

// Update welcome screen case count after cases load
function updateWelcomeStats() {
  const casesEl = document.getElementById("wlCasesNum");
  if (casesEl && cases.length > 0) casesEl.textContent = cases.length;
}
...
function showScreen(screenId) {
  ...
  // Track current screen for session restore on refresh
  sessionStorage.setItem("currentScreen", screenId);

  // Clear admin/speaker session when returning to welcome or login screens
  if (screenId === "welcome" || screenId === "adminLogin" || screenId === "speakerLogin") {
    sessionStorage.removeItem("adminCode");
    sessionStorage.removeItem("adminRole");
  }

  // Show progress bar only during candidate course screens
  const courseScreens = ["caseIntro","question","feedback","results","review","leaderboard","waiting","waitingForStart","caseConclusion","caseLeaderboard"];
  const progressBar   = document.getElementById("progressBar");
  const progressLabel = document.getElementById("progressLabel");
  if (progressBar)   progressBar.style.display   = courseScreens.includes(screenId) ? "" : "none";
  if (progressLabel) progressLabel.style.display = courseScreens.includes(screenId) ? "" : "none";
...
function startCourse() {
  ...
  // Step 1 — check email is registered
  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "validateEmail", email: emailInput })
  })
  .then(r => r.json())
  .then(emailData => {
    if (!emailData.valid) {
      ...
      if (emailErr) { emailErr.textContent = "This email is not registered. Please contact your administrator."; emailErr.style.display = "block"; }
      return;
    }
    // Step 2 — check access code
    return fetch(PROXY_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  .then(data => {
    ...
    sessionStorage.setItem("playerEmail", playerEmail);

    if (cases.length === 0) {
      if (loadingEl) loadingEl.classList.remove("active");
      if (welcomeEl) welcomeEl.classList.add("active");
      showStartError(window.caseLoadFailed
        ? "Could not load cases. Please check your connection and refresh."
  ...
  .catch(err => {
    if (loadingEl) loadingEl.classList.remove("active");
    if (welcomeEl) welcomeEl.classList.add("active");
    showStartError("Could not verify access code. Please check your connection and try again.");
  });
}

let _resultsShown = false;
function goToResults() {
  if (_resultsShown) return; // guard against double-call (e.g. timer + manual)
  _resultsShown = true;
  ...
  // Single consistent score calculation
  ...
  window.finalScoreOutOf10 = finalScore;

  // Mark course as completed by saving with caseIndex = cases.length (sentinel)
  // This lets returning users skip straight to review instead of restarting
...
function updateTopBar() {
  ...
  if (["question","caseIntro","feedback"].includes(screenId) && screenId !== "admin" && screenId !== "adminLogin") {
    ...
    crumb.innerHTML =
      ...
      '<span>Question ' + (currentQuestionIndex + 1) + ' / ' + (cases[currentCaseIndex] ? cases[currentCaseIndex].questions.length : '?') + '</span>';
  } else {
...
function startCourseTimer() {
  // Use duration from settings (default 45 min)
  const durationMins = (courseSettings && courseSettings.courseDuration) ? Number(courseSettings.courseDuration) : 45;
  courseTimeLeft = durationMins * 60;
  courseTimer = null;
...
function answerQuestion(selectedAnswer) {
  ...
  const pointsEarned    = Math.min(10, isCorrect
    ...
    : 0);
  score += pointsEarned;
  saveProgress(); // save after every answer, not just case transitions

  // Save answer distribution to server
  saveAnswerToServer(key, q, c, selectedAnswer, isCorrect);
...
function nextCase() {
  // Show conclusion of the current case before advancing
  const currentCase = cases[currentCaseIndex];
  if (currentCase && currentCase.conclusion && currentCase.conclusion.trim()) {
    showCaseConclusion(currentCase.conclusion.trim(), () => {
      advanceToNextCase();
    });
    return;
  }
...
function showCaseLeaderboard(completedIdx, onContinue) {
  const caseNum   = document.getElementById("caseLeaderboardNum");
  const tbody     = document.getElementById("caseLeaderboardBody");
  const btn       = document.getElementById("caseLeaderboardBtn");
  const myScore   = document.getElementById("caseLeaderboardMyScore");
  if (!tbody || !btn) { onContinue(); return; }

  const totalCases = cases.length;
  if (caseNum) caseNum.innerText = "CASE " + (completedIdx + 1) + " OF " + totalCases + " COMPLETE";

  // Show current player score
  const totalQ   = cases.reduce((s,c) => s + (c.questions||[]).length, 0);
  const maxSoFar = (completedIdx + 1) / totalCases * totalQ * 10;
  if (myScore) myScore.innerText = score + " pts";

  tbody.innerHTML = "<tr><td colspan='3' style='text-align:center;color:var(--muted);padding:20px;'>Loading…</td></tr>";
  btn.onclick = () => onContinue();
  showScreen("caseLeaderboard");
  ...
  .then(data => {
    const rows = (Array.isArray(data) ? data : (data.value || []))
      .map(r => ({ name: r.Name||r.name||r.Title||"—", email: r.Email||r.email||"", score: Number(r.Score||r.score||0) }))
      .sort((a,b) => b.score - a.score)
      .slice(0, 10);

    if (rows.length === 0) {
      tbody.innerHTML = "<tr><td colspan='3' style='text-align:center;color:var(--muted);padding:20px;'>No scores yet</td></tr>";
      return;
    }

    tbody.innerHTML = rows.map((r, i) => {
      const medal   = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : (i + 1) + ".";
      const isMe    = r.name === playerName || r.email.toLowerCase() === playerEmail.toLowerCase();
      const highlight = isMe ? "background:rgba(92,45,126,0.07);font-weight:600;" : "";
      return `<tr style="border-bottom:1px solid var(--border);${highlight}">
        <td style="padding:12px 8px;">${medal}</td>
        <td style="padding:12px 8px;">${escapeHtml(r.name)}${isMe ? " <span style='color:var(--accent);font-size:11px;'>(you)</span>" : ""}</td>
        <td style="padding:12px 8px;text-align:right;font-family:'JetBrains Mono',monospace;color:var(--accent);font-weight:700;">${(Math.round(r.score * 10) / 10)} pts</td>
      </tr>`;
    }).join("");
  })
  .catch(() => {
    tbody.innerHTML = "<tr><td colspan='3' style='text-align:center;color:var(--muted);padding:20px;'>Could not load scores</td></tr>";
  });
}

function showCaseConclusion(text, onContinue) {
  const body  = document.getElementById("conclusionBody");
  const btn   = document.getElementById("conclusionBtn");
  const label = document.getElementById("conclusionCaseLabel");
  if (!body || !btn) { onContinue(); return; }
  if (label) label.innerText = "CASE " + (currentCaseIndex + 1) + " OF " + cases.length;
  body.innerText = text;
  btn.onclick = () => onContinue();
  showScreen("caseConclusion");
}
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    if (dicomIsFullscreen) dicomToggleFullscreen();
    exitModalCancel();
    return;
  }

  // A/B/C/D or 1/2/3/4 to select MCQ answers
  const screen = document.querySelector(".screen.active");
  if (!screen || screen.id !== "question") return;
  if (e.target && (e.target.tagName === "INPUT" || e.target.tagName === "TEXTAREA")) return;

  const keyMap = { a:"A", b:"B", c:"C", d:"D", "1":"A", "2":"B", "3":"C", "4":"D" };
  const targetCode = keyMap[e.key.toLowerCase()];
  if (!targetCode) return;

  const item = document.querySelector('.chip-option-item[data-code="' + targetCode + '"]:not(.chip-option-disabled):not(.chip-option-hidden)');
...
function submitFreetext() {
  ...
  // Score
  const typed        = ftSelectedTags.map(t => t.toLowerCase().trim());
  const pointsPerField = 10 / numFields;
  let correctCount   = 0;
  const usedAnswers  = new Set();

  // Match helper — handles plain text AND cascading full-path (e.g. "Neoplastic Glioblastoma" matches "Glioblastoma")
  function ftMatchesCorrect(t, ca) {
    const caLo = ca.toLowerCase().trim();
    return caLo === t || t === caLo || t.endsWith(" " + caLo) || t.endsWith(" — " + caLo) || t.includes(caLo);
  }

  typed.forEach(t => {
    const matchIdx = correctAnswers.findIndex(
      (ca, ci) => ftMatchesCorrect(t, ca) && !usedAnswers.has(ci)
    );
  ...
  if (tagBox) {
    tagBox.querySelectorAll("div").forEach(tag => {
      ...
      const isCorrect = correctAnswers.some(ca => ftMatchesCorrect(lower, ca));
  ...
  const caseForAnalytics = cases[currentCaseIndex];
  typed.forEach(t => {
    const isCorr = correctAnswers.some(ca => ftMatchesCorrect(t, ca));
    saveAnswerToServer(ftKey, q, caseForAnalytics, t, isCorr);
  });
...
function submitMultiSelect() {
  if (!window.selectedCodes || window.selectedCodes.length === 0) {
    // Show warning — use chipAnswerCount for single-select (multiHint is hidden)
    const isMulti = window.chipIsMulti;
    const warningEl = isMulti
      ? document.getElementById("multiHint")
      : document.getElementById("chipAnswerCount");
    if (warningEl) {
      const original = warningEl.innerText;
      warningEl.innerText = "⚠️ Please select an answer before submitting";
      warningEl.style.color = "var(--danger)";
      warningEl.style.display = "block";
      setTimeout(() => {
        warningEl.innerText = isMulti ? "Select all correct answers, then click Submit" : "";
        warningEl.style.color = "";
      }, 2000);
    }
    // Also flash the chip input box red
    const box = document.getElementById("chipInputBox");
    if (box) {
      box.style.borderColor = "var(--danger)";
      box.style.boxShadow = "0 0 0 3px rgba(192,57,43,0.15)";
...
function goToReview() {
  ...
  cases.forEach((c, ci) => {
    ...
    function renderQuestionRow(path, depth) {
      ...
      qHeader.style.cssText = "display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;";
      ...
      ptsEl.style.whiteSpace = "nowrap";
      ptsEl.innerText = "+" + pts + " / 10 pts";
      ptsEl.style.color = pts >= 10 ? "#2e7d32" : pts > 0 ? "var(--warn)" : "var(--danger)";
      qHeader.appendChild(qText);
      qHeader.appendChild(ptsEl);
      qDiv.appendChild(qHeader);

      const isFreetext = (q.questionType || "mc").toLowerCase() === "freetext" || (q.questionType || "mc").toLowerCase() === "free";
      const userCodes  = (ua.userAnswers || []).map(a => String(a).toUpperCase());

      if (isFreetext || !q.options || q.options.length === 0) {
        // Freetext — show typed answer vs correct answer
        ...
        if (!isCorrect && correctLabels.length > 0) {
          ...
          correctTag.innerText = "✓ Correct answer";
        ...
        // Collapsible all-options list (from suggestions or options)
        // Handle plain list, JSON cascading format, or q.options
        let allOptions = [];
        if (q.options && q.options.length > 0) {
          allOptions = q.options.map(o => o.label);
        } else if (q.suggestions && q.suggestions.trim()) {
          const raw = q.suggestions.trim();
          if (raw.startsWith("[")) {
            try {
              const cascading = JSON.parse(raw);
              cascading.forEach(item => {
                if (item.label) allOptions.push(item.label);
                (item.sub || []).forEach(sub => allOptions.push(item.label + " — " + sub));
              });
            } catch(e) {
              allOptions = raw.split(/[,;\n]/).map(s => s.trim()).filter(Boolean);
            }
          } else {
            allOptions = raw.split(/[,;\n]/).map(s => s.trim()).filter(Boolean);
          }
        ...
        // Collapsible cascading options panel
        const raw = q.suggestions ? q.suggestions.trim() : "";
        const isCascading = raw.startsWith("[");
        let cascadingData = null;
        if (isCascading) { try { cascadingData = JSON.parse(raw); } catch(e) {} }

        if (allOptions.length > 0 || cascadingData) {
          const toggle = document.createElement("button");
          toggle.style.cssText = "display:flex;align-items:center;gap:6px;background:none;border:none;cursor:pointer;font-size:12px;font-weight:600;color:var(--accent);padding:6px 0;margin-top:8px;font-family:'DM Sans',sans-serif;";
          toggle.innerHTML = '<span id="ftArrow_' + key + '" style="transition:transform 0.2s;display:inline-block;">▶</span> View all available answers';

          const optPanel = document.createElement("div");
          optPanel.style.cssText = "display:none;flex-direction:column;gap:4px;margin-top:8px;border:1.5px solid var(--border);border-radius:12px;overflow:hidden;";

          // Normalize user answers and correct answers for exact matching
          const userLower    = userAnswerList.map(l => l.toLowerCase().trim());
          const correctLower = correctLabels.map(l => l.toLowerCase().trim());

          // Exact match first, then partial as fallback
          function matches(haystack, needles) {
            const lo = haystack.toLowerCase().trim();
            return needles.some(n => n === lo || lo === n || lo.includes(n) || n.includes(lo));
          }

          function getStyle(fullPath) {
            const isC = matches(fullPath, correctLower);
            const isU = matches(fullPath, userLower);
            if (isC && isU) return { bg:"rgba(46,125,50,0.1)", border:"#2e7d32", color:"#2e7d32", icon:"✅ ", badge:"Your answer · Correct" };
            if (isC)        return { bg:"rgba(46,125,50,0.07)", border:"#2e7d32", color:"#2e7d32", icon:"✓ ", badge:"Correct answer" };
            if (isU)        return { bg:"rgba(192,57,43,0.08)", border:"#c0392b", color:"#c0392b", icon:"❌ ", badge:"Your answer" };
            return { bg:"var(--surface)", border:"var(--border)", color:"var(--text)", icon:"", badge:"" };
          }

          if (cascadingData) {
            // Cascading accordion
            cascadingData.forEach((item, idx) => {
              const label  = item.label || item;
              const hasSub = item.sub && item.sub.length > 0;

              // Parent: highlight only if any sub matches, or it's a direct leaf match
              let catBg = "var(--surface2)", catColor = "var(--text)", catIcon = "";
              let anySubCorrect = false, anySubUser = false;
              if (hasSub) {
                anySubCorrect = item.sub.some(sub => matches(label + " " + sub, correctLower));
                anySubUser    = item.sub.some(sub => matches(label + " " + sub, userLower));
              } else {
                anySubCorrect = matches(label, correctLower);
                anySubUser    = matches(label, userLower);
              }
              if (anySubCorrect) { catBg = "rgba(46,125,50,0.06)"; catColor = "#2e7d32"; catIcon = "✓ "; }
              if (anySubUser && !anySubCorrect) { catBg = "rgba(192,57,43,0.05)"; catColor = "#c0392b"; }

              const catRow = document.createElement("div");
              catRow.style.cssText = `display:flex;align-items:center;gap:10px;padding:11px 16px;font-size:14px;font-weight:600;color:${catColor};background:${catBg};cursor:${hasSub ? "pointer" : "default"};border-bottom:1px solid var(--border);`;
              catRow.innerHTML = `<span style="flex:1;">${catIcon}${escapeHtml(label)}</span>${hasSub ? `<span style="font-size:11px;color:var(--muted);transition:transform 0.2s;display:inline-block;" id="rvArrow_${key}_${idx}">▶</span>` : ""}`;

              if (hasSub) {
                const subPanel = document.createElement("div");
                // Auto-open if any sub is correct or selected
                const autoOpen = anySubCorrect || anySubUser;
                subPanel.style.cssText = `display:${autoOpen ? "flex" : "none"};flex-direction:column;background:var(--surface);border-bottom:1px solid var(--border);`;
                if (autoOpen) {
                  const arr = document.getElementById("rvArrow_" + key + "_" + idx);
                  // Will be set after appending — handled below
                }

                item.sub.forEach(sub => {
                  const ss = getStyle(label + " " + sub);
                  const subRow = document.createElement("div");
                  subRow.style.cssText = `display:flex;align-items:center;gap:10px;padding:9px 16px 9px 32px;font-size:13px;color:${ss.color};background:${ss.bg};border-bottom:1px solid var(--border);border-left:3px solid ${ss.border};`;
                  subRow.innerHTML = `<span style="flex:1;">${ss.icon}${escapeHtml(sub)}</span>${ss.badge ? `<span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">${ss.badge}</span>` : ""}`;
                  subPanel.appendChild(subRow);
                });

                catRow.onclick = () => {
                  const open = subPanel.style.display === "flex";
                  subPanel.style.display = open ? "none" : "flex";
                  const arr = document.getElementById("rvArrow_" + key + "_" + idx);
                  if (arr) arr.style.transform = open ? "" : "rotate(90deg)";
                };
                optPanel.appendChild(catRow);
                optPanel.appendChild(subPanel);

                // Set arrow rotation if auto-open
                if (autoOpen) {
                  setTimeout(() => {
                    const arr = document.getElementById("rvArrow_" + key + "_" + idx);
            ...
            allOptions.forEach(label => {
              const st = getStyle(label);
              const row = document.createElement("div");
              row.style.cssText = `display:flex;align-items:center;gap:10px;padding:10px 16px;font-size:14px;color:${st.color};background:${st.bg};border-bottom:1px solid var(--border);`;
              row.innerHTML = `<span style="flex:1;">${st.icon}${escapeHtml(label)}</span>${st.badge ? `<span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">${st.badge}</span>` : ""}`;
              optPanel.appendChild(row);
            });
          }

          toggle.onclick = () => {
            const open = optPanel.style.display === "flex";
            optPanel.style.display = open ? "none" : "flex";
            const arrow = document.getElementById("ftArrow_" + key);
            if (arrow) arrow.style.transform = open ? "" : "rotate(90deg)";
          };

          qDiv.appendChild(toggle);
          qDiv.appendChild(optPanel);
        }
      } else {
        // MC / chip-select — show every option as a styled row
        const optList = document.createElement("div");
        optList.style.cssText = "display:flex;flex-direction:column;gap:6px;";

        q.options.forEach(opt => {
          const code      = (opt.code || "").toUpperCase();
          const isCorrectOpt = correctCodes.includes(code);
          const isUserOpt    = userCodes.includes(code);

          let bg = "var(--surface2)", border = "var(--border)", color = "var(--text)", icon = "";

          if (isCorrectOpt && isUserOpt) {
            bg = "rgba(46,125,50,0.1)"; border = "#2e7d32"; color = "#2e7d32"; icon = "✅ ";
          } else if (isCorrectOpt) {
            bg = "rgba(46,125,50,0.07)"; border = "#2e7d32"; color = "#2e7d32"; icon = "✓ ";
          } else if (isUserOpt) {
            bg = "rgba(192,57,43,0.08)"; border = "#c0392b"; color = "#c0392b"; icon = "❌ ";
          }

          const row = document.createElement("div");
          row.style.cssText = `display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:8px;border:1.5px solid ${border};background:${bg};font-size:14px;color:${color};`;

          const codeEl = document.createElement("span");
          codeEl.style.cssText = `font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700;min-width:22px;color:${color};opacity:0.7;`;
          codeEl.textContent = code;

          const labelEl = document.createElement("span");
          labelEl.style.flex = "1";
          labelEl.textContent = icon + escapeHtml(opt.label || "");

          const badge = document.createElement("span");
          badge.style.cssText = `font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;opacity:0.8;`;
          if (isCorrectOpt && isUserOpt) badge.textContent = "Your answer · Correct";
          else if (isCorrectOpt)         badge.textContent = "Correct answer";
          else if (isUserOpt)            badge.textContent = "Your answer";
        ...
        if (notAnswered) {
          ...
          na.style.cssText = "font-size:12px;color:var(--danger);font-weight:600;margin-top:4px;";
          na.textContent = "— Not answered";
          optList.appendChild(na);
        }
...
function resetAdminIdleTimer() {
  ...
  _adminIdleTimer = setTimeout(() => {
    ...
    if (active && (active.id === "admin" || active.id === "speaker")) {
...
function openAdmin() {
  ...
  .then(data => {
    ...
    ["mousemove","keydown","click","scroll"].forEach(ev =>
      document.addEventListener(ev, resetAdminIdleTimer, { passive: true })
    );
    showScreen("admin");
    loadAdminData();
  })
...
function loadAdminData() {
  ...
  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "getAdminData", adminCode: window.adminCode || "" })
  })
  .then(r => r.json())
  .then(data => {
...
function renderAdminStats(scores, progress) {
  ...
  const inProgress = progress.filter(p => {
    ...
    return ci >= 0 && ci < (cases.length || 9999); // exclude completed (sentinel = cases.length)
  }).length;
...
function renderAdminProgress(progress) {
  ...
  active.forEach(row => {
    ...
    const email       = row.Email || row.email || row.Title || "—";
    const rawCase     = Number(row.CaseIndex     || row.field_2 || 0);
    const questionIdx = Number(row.QuestionIndex || row.field_3 || 0) + 1;
    const score       = Number(row.Score         || row.field_4 || 0);
    if (rawCase === -1) return; // old-style skip

    const isCompleted = rawCase >= (cases.length || 1);
    const positionHtml = isCompleted
      ? `<span style="font-size:12px;background:rgba(46,125,50,0.1);color:#2e7d32;padding:3px 10px;border-radius:20px;font-weight:600;border:1px solid #2e7d32;">✓ Completed</span>`
      : `<span style="font-size:12px;background:rgba(92,45,126,0.1);color:var(--accent);padding:3px 10px;border-radius:20px;font-weight:600;">Case ${rawCase + 1} · Q${questionIdx}</span>`;
...
function filterProgressTable(mode) {
  // Update button styles
  ["All","Active","Done"].forEach(s => {
    const btn = document.getElementById("progressFilter" + s);
    if (!btn) return;
    const active = mode === s.toLowerCase();
    btn.style.background = active ? "var(--accent)" : "var(--surface)";
    btn.style.color      = active ? "#fff" : "var(--muted)";
  });

  // Show/hide rows based on whether they have the "Completed" badge
  const tbody = document.getElementById("adminProgressBody");
  if (!tbody) return;
  tbody.querySelectorAll("tr").forEach(tr => {
    const isCompleted = tr.querySelector("td:nth-child(3) span") &&
                        tr.querySelector("td:nth-child(3) span").textContent.includes("Completed");
    if (mode === "all")    tr.style.display = "";
    else if (mode === "done")   tr.style.display = isCompleted ? "" : "none";
    else if (mode === "active") tr.style.display = isCompleted ? "none" : "";
  });
}
...
function exportAdminXLSX() {
  if (!adminScoresData.length) { cmMsg("No data to export yet.", false); return; }
...
function loadCourseSettings() {
  ...
  .then(r => r.json())
  .then(data => {
    // Handle both direct object (PostgreSQL) and array (legacy SharePoint)
    const row = Array.isArray(data) ? (data[0] || {}) : (data && !data.value ? data : ((data.value || [])[0] || {}));
    courseSettings = {
      startDate:      row.startDate      || row.StartDate      || row.field_1 || null,
      endDate:        row.endDate        || row.EndDate        || row.field_2 || null,
      isActive:       row.isActive       !== undefined ? row.isActive       : (row.IsActive  !== undefined ? row.IsActive  : true),
      courseDuration: row.courseDuration !== undefined ? Number(row.courseDuration) : (row.CourseDuration !== undefined ? Number(row.CourseDuration) : 45)
    };
    // Update admin inputs if visible
    const durInput   = document.getElementById("adminCourseDuration");
    const startInput = document.getElementById("adminStartDate");
    const endInput   = document.getElementById("adminEndDate");
    const activeInput= document.getElementById("adminIsActive");
    if (durInput)    durInput.value    = courseSettings.courseDuration;
    if (startInput && courseSettings.startDate) startInput.value = courseSettings.startDate.slice(0,16);
    if (endInput   && courseSettings.endDate)   endInput.value   = courseSettings.endDate.slice(0,16);
    if (activeInput) activeInput.checked = courseSettings.isActive !== false;
...
function saveSettings() {
  ...
  const durEl           = document.getElementById("adminCourseDuration");
  const courseDuration  = durEl ? Math.max(5, Math.min(180, parseInt(durEl.value) || 45)) : 45;

  const saveBody = {
    action:         "saveSettings",
    adminCode:      window.adminCode || "",
    startDate:      startDate,
    endDate:        endDate,
    isActive:       isActive,
  ...
  .then(() => {
    courseSettings = { startDate, endDate, isActive, courseDuration };
...
function switchAdminTab(tab) {
  ...
  if (tab === "users")   loadRegisteredEmails();
}
...
function loadRegisteredEmails() {
  const list = document.getElementById("registeredEmailsList");
  if (list) list.innerHTML = "<div style='padding:16px;color:var(--muted);font-size:13px;'>Loading…</div>";

  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "getRegisteredEmails", adminCode: window.adminCode || "" })
  })
  .then(r => r.json())
  .then(data => {
    registeredEmails = Array.isArray(data.emails) ? data.emails : [];
    renderRegisteredEmails();
  })
  .catch(() => {
    if (list) list.innerHTML = "<div style='padding:16px;color:var(--danger);font-size:13px;'>Failed to load emails.</div>";
  });
}

function renderRegisteredEmails(filterQuery) {
  const list  = document.getElementById("registeredEmailsList");
  const badge = document.getElementById("emailCountBadge");
  if (badge) badge.textContent = registeredEmails.length + " email" + (registeredEmails.length !== 1 ? "s" : "");
  if (!list) return;

  const query   = (filterQuery || "").toLowerCase().trim();
  const visible = query ? registeredEmails.filter(e => e.includes(query)) : registeredEmails;

  if (registeredEmails.length === 0) {
    list.innerHTML = "<div style='padding:20px;color:var(--muted);font-size:13px;text-align:center;'>No registered emails yet. Add manually or import a CSV.</div>";
    return;
  }
  if (visible.length === 0) {
    list.innerHTML = "<div style='padding:20px;color:var(--muted);font-size:13px;text-align:center;'>No emails match your filter.</div>";
    return;
  }

  list.innerHTML = visible.map(email => {
    const realIdx = registeredEmails.indexOf(email);
    const hi = query
      ? escapeHtml(email).replace(new RegExp(escapeHtml(query), "gi"), m => `<mark style="background:rgba(92,45,126,0.18);color:var(--accent);border-radius:2px;">${m}</mark>`)
      : escapeHtml(email);
    return `<div style="display:flex;align-items:center;padding:11px 16px;border-bottom:1px solid var(--border);gap:12px;">
      <span style="font-size:13px;color:var(--text);flex:1;font-family:'DM Sans',sans-serif;">${hi}</span>
      <button onclick="removeRegisteredEmail(${realIdx})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:18px;line-height:1;padding:0 4px;opacity:0.6;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.6" title="Remove">×</button>
    </div>`;
  }).join("");
}

function filterEmailList(query) {
  renderRegisteredEmails(query);
}

function clearAllEmails() {
  if (!confirm("Clear all " + registeredEmails.length + " emails from the list?")) return;
  registeredEmails = [];
  renderRegisteredEmails();
}
...
function importEmailsFromFile(input) {
  ...
  reader.onload = function(e) {
    ...
    if (parsed.length === 0) { showUsersMsg("No valid emails found in file.", true); return; }
    ...
    if (previewText) previewText.innerHTML =
      `<strong>${parsed.length} email${parsed.length !== 1 ? "s" : ""}</strong> found in file. ` +
...
function parseEmailsFromText(text) {
  // Split on commas, semicolons, newlines, tabs — then validate each token
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return text
    .split(/[\r\n,;\t]+/)
    .map(t => t.trim().toLowerCase().replace(/^["']|["']$/g, "")) // strip quotes
    .filter(t => emailRegex.test(t));
}

function confirmImport(mode) {
  if (mode === "replace") {
    registeredEmails = [...new Set(_pendingImport)];
  } else {
    // merge — deduplicate
    const combined = [...registeredEmails, ..._pendingImport];
    registeredEmails = [...new Set(combined)];
  }
  _pendingImport = [];
  cancelImport();
  renderRegisteredEmails();
  showUsersMsg("✓ " + registeredEmails.length + " emails loaded. Click 💾 Save to apply.");
}
...
function exportEmailsCSV() {
  if (registeredEmails.length === 0) { showUsersMsg("Nothing to export.", true); return; }
  const csv = "email\n" + registeredEmails.join("\n");
  const blob = new Blob([csv], { type: "text/csv" });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement("a");
  a.href     = url;
  a.download = "registered_emails.csv";
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
...
function addRegisteredEmail() {
  const input = document.getElementById("newEmailInput");
  const val = (input.value || "").trim().toLowerCase();
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!val || !emailRegex.test(val)) { input.focus(); return; }
  if (registeredEmails.includes(val)) { showUsersMsg("Already in list."); input.value = ""; return; }
  registeredEmails.push(val);
  renderRegisteredEmails();
  input.value = "";
  input.focus();
}

function removeRegisteredEmail(i) {
  registeredEmails.splice(i, 1);
  renderRegisteredEmails();
}

function saveRegisteredEmails() {
  const msg = document.getElementById("usersMsg");
  if (msg) { msg.textContent = "Saving…"; msg.style.display = "inline"; msg.style.color = "var(--muted)"; }

  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "saveRegisteredEmails", adminCode: window.adminCode || "", emails: registeredEmails })
  })
  .then(r => r.json())
  .then(data => {
    if (data.ok) showUsersMsg("✓ Saved successfully!");
    else showUsersMsg("Error: " + (data.error || "Unknown error"), true);
  })
  .catch(() => showUsersMsg("Failed to save. Check connection.", true));
}
...
function clearAllResults() {
  if (!confirm("⚠️ This will permanently delete all SCORES, IN-PROGRESS saves and ANSWER ANALYTICS.\n\nThis cannot be undone. Are you sure?")) return;

  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "clearAllResults", adminCode: window.adminCode || "" })
  })
  .then(r => r.json())
  .then(data => {
    if (data.ok) {
      ...
      renderAdminScores([]);
      renderAdminProgress([]);
      // Also clear speaker analytics panel
      const analyticsBody = document.getElementById("answerAnalyticsBody");
      if (analyticsBody) analyticsBody.innerHTML = "<p style='color:var(--muted);font-size:13px;text-align:center;padding:24px;'>No answer data yet — data appears as candidates answer questions.</p>";
      const statusEl = document.getElementById("adminStatus");
      if (statusEl) statusEl.innerText = "✓ All data cleared.";
    } else {
      alert("Error: " + (data.error || "Unknown error"));
    }
  })
  .catch(() => alert("Failed to clear data. Check your connection."));
}
...
function cmSaveToGitHub(onSuccess) {  // name kept for compatibility
  ...
  fetch(PROXY_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "saveQuestions", adminCode: window.adminCode || "", questions: payload })
  })
  .then(r => {
    if (!r.ok) {
...
function cmCaseForm(c) {
  ...
  return `
    ...
    <div class="cm-field">
      <label class="cm-label">Conclusion (shown to candidates when they finish this case)</label>
      <textarea class="cm-textarea" id="cmF_conclusion" placeholder="e.g. This case illustrates a classic presentation of pulmonary embolism. The key imaging finding was…" style="min-height:100px;">${escapeHtml(c.conclusion||"")}</textarea>
    </div>
...
function cmSaveCase(isEdit) {
  ...
  const conclusion = (document.getElementById("cmF_conclusion") ? document.getElementById("cmF_conclusion").value.trim() : "");
...
function openSpeaker() {
  ...
  .then(data => {
    ...
    window.adminCode = code; // needed for analytics requests
    sessionStorage.removeItem("playerName");
    sessionStorage.removeItem("playerEmail");
    sessionStorage.setItem("adminCode", code);
    sessionStorage.setItem("adminRole", "speaker");
    showScreen("speaker");
    speakerRefresh();
  })
...
function switchSpeakerTab(tab) {
  document.querySelectorAll(".admin-tab").forEach(btn => {
    btn.classList.toggle("active", btn.getAttribute("onclick") === `switchSpeakerTab('${tab}')`);
  });
  const live      = document.getElementById("speakerTabLive");
  const analytics = document.getElementById("speakerTabAnalytics");
  if (live)      live.style.display      = tab === "live"      ? "" : "none";
  if (analytics) analytics.style.display = tab === "analytics" ? "" : "none";
  if (tab === "analytics") loadAnswerAnalytics();
}
...
function _enterCourse(saved) {
  if (saved && saved.answers && saved.answers !== "{}" && saved.caseIndex !== -1) {
    ...
    // Always keep the name the user just typed — don't revert to the old saved name
    playerName = playerName || saved.name;

    // Course already completed — show results screen with saved score
    if (currentCaseIndex >= cases.length) {
      _resultsShown = true;
      const totalQ    = totalQuestions();
      const maxScore  = totalQ * 10;
      ...
      if (el("finalScore"))    el("finalScore").innerText    = finalScore + " / 10";
      if (el("finalName"))     el("finalName").innerText     = playerName;
      if (el("resultMessage")) el("resultMessage").innerText = message;
      if (el("rcBadge"))       el("rcBadge").textContent     = badge;
      if (el("rcName"))        el("rcName").textContent      = playerName;
      if (el("rcScore"))       el("rcScore").textContent     = finalScore + " / 10";
      if (el("rcMessage"))     el("rcMessage").textContent   = message;
      if (el("rcRank"))        el("rcRank").textContent      = "⭐ " + rankLabel;
